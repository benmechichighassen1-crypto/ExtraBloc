<x-layouts.app>
    ...
    @forelse($declarations as $item)<tr>
        ...
        <td>@if($item->HeureEmploiDebut1){{ $item->HeureEmploiDebut1 }} - {{ $item->HeureEmploiFin1 }}@if($item->HeureEmploiDebut2)<br>{{ $item->HeureEmploiDebut2 }} - {{ $item->HeureEmploiFin2 }}@endif@else <span class="muted">Non renseigné</span>@endif</td>
        <td>@if($item->HeurePointageEntree){{ $item->HeurePointageEntree }}<br>{{ $item->HeurePointageSortie }}@else <span class="muted">Aucun pointage</span>@endif</td>
        <td>{{ $item->declared_by_username }}<br><span class="muted">{{ $item->declared_at }}</span></td>
        ...
        </form>@else <strong>{{ $item->valide_par_username }}</strong><br><span class="muted">{{ $item->valide_le }}</span>@endif</td>
