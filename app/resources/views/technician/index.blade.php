<x-layouts.app>
    ...
    @if($dossier !== '')
        <section class="card">
            ...
            @else <table><thead><tr><th>Acte</th><th>Salle</th><th>Anesthésie</th><th>Intervenants de l’acte</th><th>Action</th></tr></thead><tbody>
            @foreach($acts as $act)<tr>
                <td><strong>{{ $act->LibelleActe }}</strong><br><span class="muted">{{ $act->TypeActe }} · {{ $act->CodeActe }}</span></td>
                <td>{{ $act->Salle }}</td><td>{{ $act->HDAnest }}<br>{{ $act->HFAnest }}</td><td>
                    ...
                    @forelse($participants->get($act->NumIntv, collect()) as $participant)
                        ...
                        <label style="display:block;margin-bottom:5px"><input type="checkbox" name="cod_interv[]" value="{{ $participant->CodInterv }}" @disabled($already)> {{ $participant->DesInterv }} <span class="muted">({{ $participant->RoleIntervenant }})</span> @if($already)<span class="badge">{{ $already->statut }}</span>@endif</label>
